...
def load_data(datafile, num_clss, save=False, save_path='dataset.pkl'):
    ...
    for line in train_list:
        ...
        np_img = np_img.reshape([227,227,1])
...
def train(network, X, Y):
    ...
    model.fit(X, Y, n_epoch=1000, validation_set=0.1, shuffle=True ,show_metric=True, batch_size=64, snapshot_step=200,snapshot_epoch=False,
...
if __name__ == '__main__':
    X, Y = load_data('scene_15_train_list.txt', 15)
    #X, Y = load_from_pkl('dataset.pkl')
    net = create_alexnet(15)
    train(net,X,Y)